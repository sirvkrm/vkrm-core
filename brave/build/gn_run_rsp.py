...
def _absolutize_path_entries(path_value):
    parts = []
    for part in path_value.split(os.pathsep):
        if not part or os.path.isabs(part):
            parts.append(part)
        else:
...
def main():
    ...
    env = os.environ.copy()
    if 'PATH' in env_vars:
        # Some generated rsp files use relative PATH entries. Convert them to
        # absolute paths so child tools that change directories can still find
        # the intended toolchain binaries.
        env_vars['PATH'] = _absolutize_path_entries(env_vars['PATH'])
    env.update(env_vars)
