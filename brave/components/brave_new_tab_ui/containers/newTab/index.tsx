...
import {
  BraveTalkWidget as BraveTalk, Clock, EditTopSite, OverrideReadabilityColor
...
import {
  VKRM_BACKGROUND_ANIMATION_DARK,
  VKRM_BACKGROUND_ANIMATION_LIGHT
} from './vkrmAnimation'
...
function AnimatedHomepageBackground () {
    ...
    <>
      <div style={{
        ...
        background: isDarkMode
          ? 'radial-gradient(circle at 18% 18%, rgba(144, 224, 239, 0.14), transparent 32%), radial-gradient(circle at 78% 22%, rgba(0, 180, 216, 0.12), transparent 26%), radial-gradient(circle at 52% 76%, rgba(2, 62, 138, 0.22), transparent 34%)'
          : 'radial-gradient(circle at 18% 18%, rgba(202, 240, 248, 0.38), transparent 32%), radial-gradient(circle at 78% 22%, rgba(144, 224, 239, 0.28), transparent 26%), radial-gradient(circle at 52% 76%, rgba(0, 119, 182, 0.2), transparent 34%)'
      }}/>
      <div style={{
        position: 'fixed',
        inset: '-12%',
        ...
        filter: isDarkMode ? 'saturate(1.08)' : 'saturate(1.12)'
      }}>
        <LottiePlayer
          animationUrl={
            isDarkMode
...
class NewTabPage extends React.Component<Props, State> {
  ...
  render () {
    ...
    const shouldShowAnimatedBackground =
      !hasImage && !hasSponsoredRichMediaBackground && !colorForBackground
    ...
    return (
      <Page.App
        ...
        {shouldShowAnimatedBackground && <AnimatedHomepageBackground />}
