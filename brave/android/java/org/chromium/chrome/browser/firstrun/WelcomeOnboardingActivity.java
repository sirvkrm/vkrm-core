...
public class WelcomeOnboardingActivity extends FirstRunActivityBase
    ...
    private static final String GITHUB_URL = "https://github.com/sirvkrm/vkrm-chromium";
    ...
    private void enableWebDiscoverPreference() {
        if ((isWDPSettingAvailable()
                        && mCurrentOnboardingPage == CurrentOnboardingPage.SEARCH_PAGE)
                || isVariantB()) {
    ...
    private void finalStepAndOpenGithubTab(final boolean showSearchWidgetPromoPanel) {
        if (showSearchWidgetPromoPanel) {
            ChromeSharedPreferences.getInstance()
                    .writeBoolean(OnboardingPrefManager.SHOULD_SHOW_SEARCH_WIDGET_PROMO, true);
        }
        ...
        try {
            Intent githubIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(GITHUB_URL));
            githubIntent.addCategory(Intent.CATEGORY_BROWSABLE);
            githubIntent.setPackage(getPackageName());
            githubIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
            startActivity(githubIntent);
        } catch (ActivityNotFoundException e) {
            Log.e(TAG, "Unable to launch GitHub repo after onboarding.", e);
        }
        finish();
    }
    ...
    private void handleSetAsDefaultStep() {
        mCurrentOnboardingPage = CurrentOnboardingPage.WELCOME_PAGE;
        showWelcomePage();
    }
    ...
    private void handleAnalyticsConsentPage() {
        mCurrentOnboardingPage = CurrentOnboardingPage.TRANSPARENCY_PAGE;
        ...
        if (mTvDefault != null) {
            mTvDefault.setMovementMethod(null);
            mTvDefault.setText(R.string.vkrm_onboarding_transparency_text);
        }
        if (mIvArrowDown != null) {
            mIvArrowDown.setVisibility(View.GONE);
        }
        if (mIvBrave != null) {
            mIvBrave.setVisibility(View.GONE);
        }

        if (mTvCard != null) {
            mTvCard.setText(R.string.vkrm_onboarding_transparency_title);
        }
        if (mBtnPositive != null) {
            mBtnPositive.setText(R.string.vkrm_onboarding_get_started);
        }
        if (mBtnNegative != null) {
            mBtnNegative.setText(R.string.vkrm_onboarding_help_cta);
    ...
    private void showWelcomePage() {
        ...
        if (mTvCard != null) {
            mTvCard.setText(R.string.vkrm_onboarding_welcome_title);
            mTvCard.setVisibility(View.VISIBLE);
        }
        if (mTvDefault != null) {
            mTvDefault.setText(R.string.vkrm_onboarding_welcome_text);
            mTvDefault.setMovementMethod(null);
            mTvDefault.setVisibility(View.VISIBLE);
        }
        if (mBtnPositive != null) {
            mBtnPositive.setText(R.string.continue_text);
        }
        if (mBtnNegative != null) {
    ...
    private void showSearchPage() {
        ...
        if (mTvCard != null) {
            mTvCard.setText(R.string.wdp_title);
            mTvCard.setVisibility(View.VISIBLE);
        }
        if (mTvDefault != null) {
        ...
        if (mBtnPositive != null) {
            mBtnPositive.setText(R.string.sure_ill_help_onboarding);
        }
        if (mBtnNegative != null) {
            mBtnNegative.setText(R.string.maybe_later);
            mBtnNegative.setVisibility(View.VISIBLE);
        }
        if (mCheckboxCrash != null) {
            mCheckboxCrash.setVisibility(View.GONE);
    ...
    public void triggerLayoutInflation() {
        ...
        if (mBtnPositive != null) {
            mBtnPositive.setOnClickListener(
                    view -> {
                        if (mCurrentOnboardingPage == CurrentOnboardingPage.SEARCH_PAGE) {
                            showSearchEngineChoiceDialog();
                        } else if (mCurrentOnboardingPage
                                == CurrentOnboardingPage.TRANSPARENCY_PAGE) {
                            finalStep(false);
                        } else {
                            nextOnboardingStepForDefaultVariant();
                        }
            ...
            mBtnNegative.setOnClickListener(
                    view -> {
                        if (mCurrentOnboardingPage
                                == CurrentOnboardingPage.TRANSPARENCY_PAGE) {
                            finalStepAndOpenGithubTab(false);
                        } else {
                            nextOnboardingStepForDefaultVariant();
                        }
                    });
    ...
    private void showSearchEngineChoiceDialog() {
        ...
        java.util.List<TemplateUrl> templateUrls = templateUrlService.getTemplateUrls();
        ...
        CharSequence[] engineNames = new CharSequence[templateUrls.size()];
        int defaultIndex = 0;
        for (int i = 0; i < templateUrls.size(); i++) {
            TemplateUrl templateUrl = templateUrls.get(i);
            engineNames[i] = templateUrl.getShortName();
            if ("Brave".equalsIgnoreCase(templateUrl.getShortName())
                    || templateUrl.getKeyword().toLowerCase(Locale.ROOT).contains("brave")) {
                defaultIndex = i;
            }
        }

        final int[] checkedIndex = new int[] {defaultIndex};
        new AlertDialog.Builder(this)
                .setTitle(R.string.search_engine_dialog_title)
                .setSingleChoiceItems(
                        engineNames,
                        defaultIndex,
                        (dialog, which) -> checkedIndex[0] = which)
                .setPositiveButton(
                        R.string.ok,
                        (dialog, which) -> {
                            TemplateUrl selectedTemplateUrl = templateUrls.get(checkedIndex[0]);
                            String shortName = selectedTemplateUrl.getShortName();
                            ChromeSharedPreferences.getInstance()
                                    .writeString(
                            ...
                            ChromeSharedPreferences.getInstance()
                                    .writeString(
                                            BraveSearchEngineAdapter.PRIVATE_DSE_SHORTNAME,
                                            shortName);
                            templateUrlService.setSearchEngine(selectedTemplateUrl.getKeyword());
                            enableWebDiscoverPreference();
                            nextOnboardingStepForDefaultVariant();
                        })
                .setNegativeButton(
                        R.string.vkrm_search_choice_close,
